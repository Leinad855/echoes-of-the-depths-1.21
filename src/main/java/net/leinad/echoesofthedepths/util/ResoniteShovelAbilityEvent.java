...
public class ResoniteShovelAbilityEvent implements PlayerBlockBreakEvents.Before{
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity) {
        ...
        if (mainHandItem.getItem() instanceof ResoniteShovelItem shovel && mainHandItem.get(ModDataComponentsType.CHARGE) == ResoniteShovelItem.MAX_CHARGE && player instanceof ServerPlayerEntity serverPlayer) {
            if (HARVESTED_BLOCKS.contains(pos)) {
                return true;
            }

            for (BlockPos position: ResoniteShovelItem.getBlocksToBeDestroyed(8, pos, serverPlayer)) {
                if (pos == position || !shovel.isCorrectForDrops(mainHandItem, world.getBlockState(position))) {
                    continue;
                }
